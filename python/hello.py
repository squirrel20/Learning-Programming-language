...
my_list = squares
my_list.append(1)			# 把一个元素添加到列表的结尾
my_list.extend([2,3])		# 白一个列表添加到list的结尾
my_list.insert(0, 2)		# 在指定位置（0）插入一个元素（2）
my_list.remove(1)			# 删除列表中值为1的第一个元素，如果没有这个元素，返回错误
my_list.pop()				# 返回并删除最后一个元素
my_list.pop(1)				# 返回并删除第2个元素
my_list.index(1)			# 返回列表中第一额值为x的元素的索引
my_list.count(1)			# 返回列表中1出现的次数
my_list.sort()				# 对列表中的元素就地排序
my_list.reverse()			# 就地倒排列表中的元素
my_list.clear()				# 从列表中删除所有元素

# 列表推导式
# 列表推导式由包含一个表达式的括号组成，表达式后跟随一个for子句，
# 之后可以有零或多个for或if子句
...
#-------------------------------------------------------------------------------
a = [-1, 1, 22, 33, 44]
del a[0]	# a = [1, 22, 33, 44] 
del a[2:4]	# a = [1, 22]
del a[:]	# a = []
del a		# 删除整个变量，再引用a会引发错误

#-------------------------------------------------------------------------------
# 元组，标准序列类型，由数个逗号分隔的值组成
#-------------------------------------------------------------------------------
t = 12345, 54321, 'hello!'	# t[0] = 12345, t = (12345, 54321, 'hello!')
# t[0] = 888	# 不可以给元组的一个独立的元素赋值
x, y, z = t

empty = ()	# 空元组
singleton = 'hello',	# 单元组，后面跟一个逗号
...
empty = set()	# 空set
a = set('abcde')	# {'a', 'b', 'c', 'd', 'e'}
b = set('cdefg')	# {'c', 'd', 'e', 'f', 'g'}
a - b	# letters in a but not in b {'a', 'b'}
a | b	# letters in either a of b {'a', 'b', 'c', 'd', 'e', 'f', 'g'}
a & b	# letters in both a and b {'c', 'd', 'e'}
a ^ b	# letters in a or b but not both {'a', 'b', 'f', 'g'}
...
tel['jack']	# 4098
del tel['sape']	# tel = {'jack': 4098}
tel['guido'] = 4217	# tel = {'guido': 4217, 'jack': 4098}
list(tel.keys())	# ['guido', 'jack']
